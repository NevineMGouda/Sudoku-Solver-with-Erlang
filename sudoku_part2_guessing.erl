...
%% that might exist.
%% -------------------------------------------------------------------
-module(sudoku_part2_guessing).

-export([benchmarks/0, par_benchmarks/0, solve_all/0, solve/1]).
...
collect_results(Solution, 0)    -> Solution;
collect_results(Solution,Count) ->
    receive
        ProblemSolved -> collect_results( lists:append( Solution, [ProblemSolved] ), Count-1 )
    end.
...
par_benchmarks()->
    ...
    Pid = self(),
    [ register(Name, spawn( fun() -> Pid ! { Name, bm( fun() -> solve(M) end) } end ) ) || {Name, M} <- Problems ],
    timer:tc(fun () -> collect_results([],length(Problems))end).
...
case solved(M) of
    ...
    false ->
      ...
      [ spawn (
          fun() ->
                  case p_solve( PossibleSolution ) of
                      no_solution -> skip ;
                      Solution ->
                          Pid ! Solution
                  end
          end )
        || PossibleSolution <- guesses(M)
      ],
      receive
       Solution -> Solution
      end
